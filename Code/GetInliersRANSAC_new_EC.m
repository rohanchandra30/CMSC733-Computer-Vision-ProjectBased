function [y1,y2,index,F] = GetInliersRANSAC_new_EC(x1,x2, ind_x)
%% Input:
...
% Choose 8 correspondences, x?1 and x?2 randomly 
  ...
  S = [];
  for j=1:msize
    if constraint(cell2mat(x2(ind_x(j),:)), f, cell2mat(x1(ind_x(j),:))) < 1e-2
        constraint(cell2mat(x2(ind_x(j),:)), f, cell2mat(x1(ind_x(j),:)))
        S = [S; ind_x(j)];
    end
  end
